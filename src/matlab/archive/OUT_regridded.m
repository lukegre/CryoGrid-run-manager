...
%         function out = store_OUT(out, t, TOP, BOTTOM, forcing, run_number, timestep, result_path)
        ...
        function out = add_forcing(out, tile)
            forcing = tile.FORCING.DATA;
            time_f = forcing.timeForcing;
            time_o = out.timestamp;
            
            idx = ismember(time_f, time_o);
            
            fields = fieldnames(forcing);
            % drop timeForcing for fields
            fields = fields(~strcmp(fields, 'timeForcing'));

            for i = 1:size(fields,1)
                name = fields{i,1};
                values = forcing.(name);
                subset = values(idx);
                out.result.(["forcing_" fields{i,1}]) = subset;
            end
        end

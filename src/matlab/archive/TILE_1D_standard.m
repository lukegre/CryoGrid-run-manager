...
%             BOTTOM_CLASS = tile.BOTTOM_CLASS;
            ...
            %TIME INTEGRATION
            %=========================================================================
            counter_exceeds_threshold = 0;
            counter = 0;
            timesteps = [];
            while tile.t < tile.FORCING.PARA.end_time
                counter = counter + 1;
                tile.cycle_counter = tile.cycle_counter + 1;
                % disp(string(tile.t))  % DEBUGGING
                ...
                timesteps = [timesteps, tile.timestep];

                % if timestep is the same as last time and the same as the threshold, then raise an error
                min_timestep_threshold = 0.01;
                max_iterations = 1000;
                
                if tile.timestep < min_timestep_threshold
                    has_tag = isfield(tile.OUT.PARA, 'tag');
                    if has_tag
                        cluster_id = tile.OUT.PARA.tag;
                    else
                        cluster_id = 'undefined';
                    end
                    counter_exceeds_threshold = counter_exceeds_threshold + 1;
                    if counter_exceeds_threshold > max_iterations
                        
                        message = strcat( ...
                            '[CLUSTER_ID = %s] After %d timesteps, the step increments continue ', ...
                            'to be < %.2f seconds for %d iterations. ', ...
                            'Average timestep is %.3f seconds. Median timestep is %.3f seconds.', ...
                            'Saving tile for debugging and aborting.');
                        save(['error_dump-tile-ID' cluster_id], 'tile')
                        error(message, cluster_id, counter, min_timestep_threshold , max_iterations, mean(timesteps), median(timesteps))
                    end
                end

                if tile.t >= tile.next_break_time
                    counter_exceeds_threshold = 0;
                end
            ...
            disp(sprintf('average timestep = %.2f sec; median timestep = %.3f sec\n', mean(timesteps), median(timesteps)))

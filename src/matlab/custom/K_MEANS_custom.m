...
classdef K_MEANS_custom< matlab.mixin.Copyable
    ...
    methods
        function cluster = provide_PARA(cluster)
            ...
            cluster.PARA.cluster_variable_scaling = [];  % ability to force a variable to me more important than the others
            cluster.PARA.cluster_variable_scaling_index = []; 
        end
        ...
        function cluster = finalize_init(cluster)
         ...
         function cluster = compute_clusters(cluster)
            n_clusters = cluster.PARA.number_of_clusters;
            ...
            % LUKE: if custom scaling is provided for each of the variables, then apply those to then Zscore normalised data here
            scaling = ones(1, size(Zs, 2));
            % if these are not explicitly set, then reruns the whole config step for some reason 
            scaling_index = cluster.PARA.cluster_variable_scaling_index;
            scaling_factor = cluster.PARA.cluster_variable_scaling;
            for k = 1:size(cluster.PARA.cluster_variable_scaling_index)
                scaling(1, scaling_index(k)) = scaling_factor(k);
            end
            
            disp(sprintf('applying scaling to normalized columns: [%s]', num2str(scaling)))
            Zs = Zs .* scaling;
            
            % Note, may require many iterations (more than standard 1e2) for large Nc
            rng(size(data_cube,2))
            disp(sprintf('computing clusters [k=%d]', n_clusters));

...
def get_profile_properties(profile: xr.DataArray)->xr.Dataset:
    ...
    ds['bottom_thawing_depth'] = detect_active_layer_depth(~bottom_thawing.groupby('time.year').max()).where(lambda x: x != 0)
    ds['permafrost_thickness'] = ds.active_layer_depth - ds.bottom_thawing_depth.fillna(depth.min())
...
def detect_active_layer(ground_temperature: xr.DataArray)->xr.DataArray:
    ...
    active_layer_depth = (
        depth
        .where(max_temp_annual)  # broadcast depth to thawed_annual
        .pipe(lambda x: x * 0 + 1)
        .ffill('depth')
        .fillna(0)
        .astype(bool)
...
def detect_active_layer_depth(active_layer_mask: xr.DataArray)->xr.DataArray:
    ...
    active_layer_depth = (
        depth
        .where(active_layer_mask)
        .fillna(0)  # fill with 0 where thawed_annual is False
        .idxmin('depth')  # get the deepest thawed layer
        .where(lambda x: x != depth.min())  # if the deepest thawed layer is max depth, then no active layer
...
def detect_bottom_thawing(ground_temperature: xr.DataArray, min_frozen_frac=0, upper_limit=-5)->xr.DataArray:
    """
    ...
    groups = (  # a new group is defined each time the temperature crosses 0 (+ve or -ve)
        frozen 
        .diff(dim='depth')  # compute where the freezing starts and ends

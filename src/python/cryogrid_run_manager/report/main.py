...
def create_report(experiment_path, report_path=None, with_profile_plots=True):
    ...
    from .excel import get_max_depth, get_excel_config
...
def make_interactive_map(fname_spatial, crs, fname_profiles=None, fname_excel=None, profile_figure_path=None):
    ...
    if fname_profiles is not None:
        logger.info(f"Adding profile locations to map")
        df = get_profiles_as_gdf(fname_profiles, fname_spatial)
        if profile_figure_path is not None:
            df['image'] = df.index.map(lambda x: f"<img src='{profile_figure_path}/{x}.png' width=800>")
            for row in df.index:
...
def add_profiles_to_map(df, m=None):
    from cryogrid_pytools.viz import (
        make_tiles, 
        MARKER_STYLES as marker_styles)
    from cryogrid_pytools.viz.folium_helpers import (
    ...
    m = plot_gdf(df_success, m=m, name=f'Profiles: success ({n_success})', **marker_styles.blue_circle['style_kwds'])
    m = plot_gdf(df_empty, m=m, name=f'Profiles: empty ({n_empty})', **marker_styles.orange_circle['style_kwds'])
    m = plot_gdf(df_failed, m=m, name=f'Profiles: failed ({n_failed})', **marker_styles.red_circle['style_kwds'])

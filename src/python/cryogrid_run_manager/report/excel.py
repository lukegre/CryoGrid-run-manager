...
def get_excel_config(fname_excel):
    ...
    return CryoGridConfigExcel(fname_excel, check_strat_layers=False, check_file_paths=False)
...
def get_stratigraphy_info(fname_excel):
    ...
    strat_info = config.get_class("STRAT_layers").T.apply(lambda x: pd.DataFrame(x.iloc[0][0]).iloc[0], axis=1)
    strat_info.index = strat_info.index.str.replace("STRAT_layers_", "").str.zfill(2)
    
    return strat_info


def get_max_depth(fname_excel, var_name='depth_below_ground', class_name='OUT_regridded_FCI2'):
    from cryogrid_pytools import CryoGridConfigExcel

...
def main(run_path, bbox_WSEN, template_dir, n_land_classes=3):
    ...
    fpath_bbox, fpath_config = make_data_for_run(run_path, n_land_classes=n_land_classes)
    make_forcing_plots(run_path)

    return fpath_bbox, fpath_config


def make_run_folder_structure(run_path: Union[str, pathlib.Path], bbox_WSEN: list[float], template_dir: Union[str, pathlib.Path]):
    """
...
def make_data_for_run(run_path, n_land_classes=3):
    ...
    ds_geo.to_netcdf(forcing_path / 'geospatial_data.nc', encoding={k: {'zlib': True} for k in ds_geo.data_vars})
...
def make_forcing_plots(run_path):
    ...
    fig_geo.savefig(run_path / 'figures' / 'geospatial_data.png', dpi=300, bbox_inches='tight')
...
def copy_template_file(src_fname, dst_fname):
    ...
    assert dst_fname.parent.exists(), f"Could not find the parent folder for {dst_fname}"
...
def make_run_cryogrid(run_path, template_dir):
    ...
    rendered_str = env.from_string(raw_str).render(
        run_name=run_name,
        username=username)
...
def get_bbox(fpath:pathlib.Path)->list[str]:
    ...
    try:
        ...
        if 'W' in str(e):
            raise TypeError(f"Could not read {fpath} as floats for the following: {bbox}")
...
def get_era5_from_s3_bucket(bbox:tuple, time_start:str, time_end:str):
    ...
    ds_central_asia = xr.open_zarr(fname_era5_s3)  # make sure that .env is set up correctly

    logger.info(f"Downloading ERA5 data for bbox {bbox} from {time_start} to {time_end}")

    ds = (
        ds_central_asia  # contains all central asia data from 1940 to 2024-10-01
        .sel(time=slice(time_start, time_end))  # there are some values in 1959-01-01 that are nans
    ...
    if ds_bbox.latitude.size == 0 or ds_bbox.longitude.size == 0:
        raise ValueError(f"Could not find any data for bbox {bbox} from {time_start} to {time_end}")
    
    if ds_bbox.latitude.size <= 1 or ds_bbox.longitude.size <= 1:
        logger.warning(f"Only one point found for bbox {bbox}, expanding the bbox to include more data")
        ...
        ds_bbox = ds.rio.clip_box(*bbox, crs='epsg:4326').isel(latitude=slice(0, 2), longitude=slice(0, 2))
...
def get_geospatial_data(bbox:tuple, n_land_classes=3)->xr.Dataset:
    ...
    from .stratigraphy import calc_surface_classes_4, calc_surface_classes_3
...
def plot_geospatial_data(ds:xr.Dataset)->tuple[plt.Figure, plt.Axes]:
    ...
    aspect = ds.elevation.sizes['x'] / ds.elevation.sizes['y'] * 1.15  # account for colorbar
    w = 11
    h = w / aspect
    ...
    imgs += ds.elevation.plot(ax=axs[1], cmap='terrain', robust=True, add_colorbar=False),
    imgs += ds.slope.plot(ax=axs[2], robust=True, add_colorbar=False),  
    imgs += ds.surface_classes.plot(ax=axs[3], cmap='RdYlBu_r', levels=np.arange(-0.5, n_surface_classes), add_colorbar=False),
    ...
    fig.tight_layout()
    cbar1 = plt.colorbar(imgs[1], ax=axs[:2], label='Elevation [m]', pad=0.01, location='right', fraction=0.05)
    cbar3 = plt.colorbar(imgs[3], ax=axs[2:], label='Surface class', pad=0.01, location='right', fraction=0.05)

    ticks = [float(f) for f in range(n_surface_classes)]
    cbar3.set_ticks(ticks)
    cbar3.set_ticklabels([
        f"{i}) {n}" for i, n in 
            zip(ds.surface_classes.attrs['class_values'], 
...
def plot_html_bounds(ds: xr.Dataset):
    ...
    m = df.explore(column='names', **GOOGLE_TERRAIN)

...
def calc_surface_classes_4(dem:xr.DataArray, land_cover:xr.DataArray, glaciers:xr.DataArray) -> xr.Dataset:
...
def calc_surface_classes_3(dem:xr.DataArray, land_cover:xr.DataArray, glaciers:xr.DataArray) -> xr.Dataset:
    """Compute stratigraphy from a DEM"""
    import numpy as np
    import xrspatial as xrs
...
def plot_stratigraphy_count_in_clusters(fname_spatial_info_mat:str):
    ...
    ds_flat = cg.read_mat_struct_as_dataset(**props).assign_coords(index=lambda x: x.index + 1)
    ...
    axs["b"].set_title(f'b) Stratigraphy index', **props)
    axs["b"].set_ylabel('')

    data_c = ds_flat.sel(index=idx.values).stratigraphy_index.to_series().value_counts()
    data_c.plot.bar(width=0.8, ax=axs["c"])
    # adjusting plot
    axs["c"].set_title(f'c) Stratigraphy class dist. in clusters (k={idx.size})', loc='left', color='0.5')

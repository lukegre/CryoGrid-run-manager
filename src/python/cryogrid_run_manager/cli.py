...
@click.option('--bbox', '-b', required=True, callback=parse_bbox, help='Bounding box as comma-separated values (W,S,E,N)')
@click.option('--template-dir', '-t', default='templates', help='Directory containing template files')
def create_new_run(name, bbox, template_dir):
    ...
    assert run_dir.exists(), f"Run directory {run_dir} does not exist - check that you're in the somewhere in CryoGrid-run-manager project directory"
...
@click.command()
@click.option('--experiment-path', '-e', type=click.Path(), default='.', help='Path to the experiment folder that contains the excel config')
@click.option('--report-path', '-o', type=click.Path(), help='Output path for the report (e.g., ./report.html)')
@click.option('--no-profile-plots', '-n', is_flag=True, help='Do not create profile plots in popups')
def create_report(experiment_path, report_path, no_profile_plots):
    ...
    m = create_report(experiment_path, report_path=report_path, with_profile_plots=not no_profile_plots)
